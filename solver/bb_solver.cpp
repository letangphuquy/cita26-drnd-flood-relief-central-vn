...
struct BoundsCache {
  ...
  // min_time[ki][ii][si] = min travel time from demand ii to hub ki in scenario
  // si
  ...
  explicit BoundsCache(const DRNDInstance &inst) : inst(inst) {
    ...
    min_time_ki.assign(
        H, vector<vector<double>>(I, vector<double>(S, inst.big_M)));
    for (int ki = 0; ki < H; ki++) {
      ...
      for (int ii = 0; ii < I; ii++) {
        ...
        for (int si = 0; si < S; si++) {
          ...
          for (int m = 0; m < M; m++)
            if (sc.acc(m, i, k))
              min_time_ki[ki][ii][si] =
                  std::min(min_time_ki[ki][ii][si], inst.C_time[m][i][k]);
        }
      }
    }
    ...
    for (int ii = 0; ii < I; ii++) {
      for (int si = 0; si < S; si++) {
        ...
        for (int ki = 0; ki < H; ki++) {
          ...
          double omega =
              (t < inst.big_M) ? sc.hub_process_time[ki] + 2.0 * t : inst.big_M;
  ...
  double lb_Z1(const vector<int> &X, int depth) const {
    ...
    for (int ii = 0; ii < I; ii++) {
      ...
      for (int ki = 0; ki < H; ki++) {
        ...
        if (feasible)
          min_et = std::min(min_et, exp_theta[ki][ii]);
      }
  ...
  // lb_Z2 = Σ_s π_s * max_ii { min_{feasible ki} D_i * expm1(λ_is *
  // min_omega_is) } "Feasible" = (committed-open OR undecided) AND risk[k] <=
  // chi in scenario si. Valid lower bound: for every solution in the subtree,
  // each demand's actual
  //   omega is >= the best omega over the same (risk-safe) feasible set, so
  //   max_ii(actual omega) >= lb, and thus actual Z2 >= lb_Z2.
  // Note: omega_sorted[ii][si] already excludes risk-unsafe hubs (see
  // constructor).
...
static void try_one(const vector<int> &X, const vector<double> &R,
  ...
  if (ind.CV < EPS) {
    ...
    // debug CV on first few failures
    static int cv_fails = 0;
    if (cv_fails++ < 5) {
      std::cerr << "DEBUG: ind.CV = " << ind.CV << "\n";
    }
  }
}
...
// ---------------------------------------------------------------------------
static void evaluate_leaf(const vector<int> &X, const DRNDInstance &inst,
                          int num_trials, ParetoArchive &archive) {
  const int H = inst.num_H, I = inst.num_I;
  ...
  for (const auto &base : snap) {
    ...
    for (int p = 0; p < PERTURB_PER_SOL; p++) {
      ...
      for (int ki : open_ki) {
        R[ki] = std::max(0.0, std::min(1.0, R[ki] + rand_gauss(SIGMA_PERTURB)));
      }
      vector<double> W = base.W;
      for (auto &w : W) {
        w = std::max(0.0, std::min(1.0, w + rand_gauss(SIGMA_PERTURB)));
      }
...
// ---------------------------------------------------------------------------
static void bb_search(vector<int> &X, int depth, const DRNDInstance &inst,
                      const BoundsCache &bc, ParetoArchive &archive,
                      int num_trials, bool use_pruning) {
  if (Duration(Clock::now() - g_t_start).count() > g_time_limit_s)
    return;
  ...
  // ── Branch ────────────────────────────────────────────────────────────
  // In BB mode, try val=1 first to find good solutions early (improves
  // pruning). In enum mode, order is irrelevant — use 1 then 0 for consistency.
  for (int val : {1, 0}) {
    X[depth] = val;
...
static void write_output(const ParetoArchive &archive, const string &out_path,
  ...
  j["all_feasible"] =
      jfront; // identical for BB-Exact (all are rank-1 feasible)
...
int main(int argc, char *argv[]) {
  ...
  cerr << "Mode     : "
       << (use_pruning ? "Branch-and-Bound (pruning ON)"
                       : "Complete Enumeration (pruning OFF)")
       << "\n";
  cerr << "Instance : " << inst_path << "\n";
  cerr << "  H=" << inst.num_H << "  I=" << inst.num_I << "  J=" << inst.num_J
       << "  S=" << inst.num_S << "\n";
  ...
  cerr << "\n";
  if (inst.num_H > 20)
    cerr << "  WARNING: num_H > 20; enumeration may not finish within the time "
            "limit.\n";
  cerr << "Trials/leaf: " << num_trials << " | Time limit: " << time_limit
       << " s\n\n";
