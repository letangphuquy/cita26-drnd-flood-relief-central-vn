...
for it, st, tt, km in cfgs:
    run = [
        ...
        '--iter', str(it), '--time-limit', '60', '--tabu-tenure', str(tt), '--kmax', str(km),
        '--starts', str(st), '--out', 'results/exp1/cv_small_vns_ts.json'
    ]
    r = subprocess.run(run, capture_output=True, text=True)
    if r.returncode != 0:
        print('FAIL', it, st, tt, km)
        continue
    ...
    print(f'iter={it} starts={st} tabu={tt} -> HV={hv:.3f} IGD={igd:.3f} wall={wall:.1f}')
